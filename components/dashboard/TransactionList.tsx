...
import { Search, Filter, ChevronLeft, ChevronRight, Calendar, Edit2, Trash2, Download, Loader2 } from 'lucide-react';
...
const CATEGORY_STYLES: Record<string, { bg: string; text: string; border: string; emoji: string }> = {
  comida:       { bg: 'bg-orange-50',  text: 'text-orange-700',  border: 'border-orange-100',  emoji: '🍕' },
  super:        { bg: 'bg-blue-50',    text: 'text-blue-700',    border: 'border-blue-100',    emoji: '🛒' },
  supermercado: { bg: 'bg-blue-50',    text: 'text-blue-700',    border: 'border-blue-100',    emoji: '🛒' },
  servicios:    { bg: 'bg-yellow-50',  text: 'text-yellow-700',  border: 'border-yellow-100',  emoji: '💡' },
  vivienda:     { bg: 'bg-yellow-50',  text: 'text-yellow-700',  border: 'border-yellow-100',  emoji: '🏠' },
  transporte:   { bg: 'bg-indigo-50',  text: 'text-indigo-700',  border: 'border-indigo-100',  emoji: '🚌' },
  salud:        { bg: 'bg-red-50',     text: 'text-red-700',     border: 'border-red-100',     emoji: '💊' },
  educacion:    { bg: 'bg-teal-50',    text: 'text-teal-700',    border: 'border-teal-100',    emoji: '🎓' },
  ocio:         { bg: 'bg-purple-50',  text: 'text-purple-700',  border: 'border-purple-100',  emoji: '🍿' },
  compras:      { bg: 'bg-pink-50',    text: 'text-pink-700',    border: 'border-pink-100',    emoji: '🛍️' },
  ropa:         { bg: 'bg-pink-50',    text: 'text-pink-700',    border: 'border-pink-100',    emoji: '👕' },
  tecnologia:   { bg: 'bg-cyan-50',    text: 'text-cyan-700',    border: 'border-cyan-100',    emoji: '💻' },
  electro:      { bg: 'bg-cyan-50',    text: 'text-cyan-700',    border: 'border-cyan-100',    emoji: '📺' },
  casa:         { bg: 'bg-amber-50',   text: 'text-amber-700',   border: 'border-amber-100',   emoji: '🏠' },
  varios:       { bg: 'bg-slate-100',  text: 'text-slate-600',   border: 'border-slate-200',   emoji: '📦' },
  otros:        { bg: 'bg-slate-100',  text: 'text-slate-600',   border: 'border-slate-200',   emoji: '📦' },
};

const getCatStyle = (id: string) => CATEGORY_STYLES[id] ?? { bg: 'bg-slate-100', text: 'text-slate-600', border: 'border-slate-200', emoji: '📦' };

// Plataforma: primero bank directo, si no installmentData.bank
const getBank = (tx: Transaction): string | null => {
  const bank = tx.bank || tx.installmentData?.bank || null;
  if (!bank || bank === 'Efectivo' || bank === '-') return null;
  return bank;
};
...
export default function TransactionList({ transactions, onEdit, onDelete, privacyMode }: TransactionListProps) {
  ...
  const generatePDF = async () => {
    ...
    autoTable(doc, {
      startY: currentY,
      head: [['Fecha', 'Descripción', 'Categoría', 'Plataforma', 'Origen', 'Monto']],
      body: tableData,
      theme: 'grid',
  ...
  const renderCalendar = () => {
    ...
    const calendarDays = [];
    for (let i = 0; i < startDay; i++) calendarDays.push(<div key={`e-${i}`} className="h-8 w-8" />);
    for (let i = 1; i <= days; i++) {
      calendarDays.push(
        <div key={i} className="h-8 w-8 flex items-center justify-center text-sm rounded-full hover:bg-blue-50 cursor-pointer font-medium text-slate-700">{i}</div>
      );
    }
    return calendarDays;
  };
    ...
    <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-6 relative z-0 mt-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <div className="relative">
            <button onClick={() => setShowDatePicker(!showDatePicker)}
              ...
              <Calendar size={16} />{monthNames[filterDate.getMonth()]} De {filterDate.getFullYear()}
            </button>
            {showDatePicker && (
              <>
                <div className="fixed inset-0 bg-black/60 z-40 md:hidden backdrop-blur-sm" onClick={() => setShowDatePicker(false)} />
                <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 md:absolute md:top-full md:left-0 md:translate-x-0 md:translate-y-2 bg-white p-5 rounded-3xl shadow-2xl border border-slate-100 w-[300px]">
          ...
          <div className="relative w-full sm:w-auto">
            <select className="w-full sm:w-[200px] appearance-none bg-white border border-slate-200 text-slate-700 px-4 py-2.5 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-black"
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm font-medium outline-none focus:ring-2 focus:ring-black transition-all"
        />
      </div>
        ...
        <table className="w-full min-w-[640px]">
          <thead>
            <tr className="text-left border-b border-slate-100">
              ...
              <th className="pb-4 text-[10px] font-black text-slate-400 uppercase tracking-wider">Plataforma</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {filteredTx.length > 0 ? (
              filteredTx.map((tx) => {
                const catStyle = getCatStyle(tx.categoryId);
                const bank = getBank(tx);
                ...
                return (
                  <tr key={tx.id} className="group hover:bg-slate-50/50 transition-colors">
                    ...
                    {/* CATEGORÍA */}
                    <td className="py-4">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold capitalize border ${catStyle.bg} ${catStyle.text} ${catStyle.border}`}>
                        {catStyle.emoji} {tx.categoryId}
                      </span>
                    </td>

                    {/* PLATAFORMA — ahora se muestra para TODOS los gastos */}
                    <td className="py-4">
                      {bank ? (
                    ...
                    {/* ORIGEN */}
                    <td className="py-4">
                      <span className={`text-[10px] font-bold px-2 py-1 rounded border uppercase tracking-wider ${tx.source === 'ahorro' ? 'bg-purple-50 text-purple-700 border-purple-100' : tx.source === 'tarjeta' ? 'bg-orange-50 text-orange-700 border-orange-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>
                        {tx.source === 'ahorro' ? 'Ahorros' : tx.source === 'tarjeta' ? 'Tarjeta' : 'Sueldo'}
                      </span>
                    </td>
              ...
              <tr>
                <td colSpan={7} className="py-12 text-center">
                  <p className="text-slate-400 text-sm font-medium">No se encontraron movimientos para esta búsqueda.</p>
                </td>
              </tr>
            )}
          </tbody>
